module Fable.Transforms.CSharp.Replacements
...
let (|CSharpInt|_|) = function
    | Int8 | UInt8 | Int16 | UInt16 | Int32 | UInt32 | Int64 | UInt64 -> Some CSharpInt
    | _ -> None

let (|CSharpDouble|_|) = function
...
let applyOp (com: ICompiler) (ctx: Context) r t opName (args: Expr list) =
    ...
    let nativeOp opName argTypes args =
        ...
        // In csharp % operator and .remainder give different values for negative numbers
...
// Mirrors Fable2CSharp.Util.equals
let rec equals (com: ICompiler) ctx r equal (left: Expr) (right: Expr) =
    ...
    | Array(t,_) ->
        ...
        // but this are not necessary because of null safety in CSharp
        | NullConst, _
...
// Mirrors Fable2CSharp.Util.compare
...
let rec getZero (com: ICompiler) (ctx: Context) (t: Type) =
    ...
    | String -> makeStrConst "" // Using empty string instead of null so CSharp doesn't complain
...
let tryReplacedEntityRef (com: Compiler) entFullName =
    ...
    | "Fable.Core.CSharp.Future`1" -> makeIdentExpr "Future" |> Some
    | "Fable.Core.CSharp.Stream`1" -> makeIdentExpr "Stream" |> Some
...
// Don't use `Exception` for now because it doesn't catch all errors in CSharp
// See Fable2CSharp.transformDeclaredType
...
let fableCoreLib (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "Fable.Core.Compiler", meth ->
        ...
        | _ -> None
    | Naming.StartsWith "Fable.Core.CSharp" rest, _ ->
        match rest with
        | ".CSharpNullable`1" ->
...
// Not sure if this is needed in CSharp, see comment in JS.Replacements.makeRefFromMutableFunc
...
let operators (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    let math r t (args: Expr list) argTypes genArgs methName =
        ...
        Helper.ImportedCall("csharp:math", meth, t, args, argTypes, genArgs=genArgs, ?loc=r)
    ...
    | ("op_Range"|"op_RangeStep"), _ ->
        ...
        let modul, meth, args =
            ...
            | Number(CSharpInt,_) -> "Range", "rangeInt", addStep args
...
//    |  "NullArg"         // nullArg
    ...
    | ("Min"|"Max"|"Clamp" as meth), _ ->
        let meth = Naming.lowerFirst meth
        match meth, t with
        | ("min"|"max"), Number((CSharpInt|CSharpDouble), NumberInfo.Empty) ->
            Helper.ImportedCall("csharp:math", meth, t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?loc=r) |> Some
        | _ ->
...
// Find/FindLast don't work because we cannot provide a default value for ref types with null safety in CSharp
//    | "Find", Some ar, [arg] ->
...
let intrinsicFunctions (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ("GetArraySlice" | "GetStringSlice" as meth), None, [ar; lower; upper] ->
        ...
        let args =
            ...
            // Upper index is inclusive in F# but exclusive in CSharp
...
//        Helper.LibCall(com, "Set", meth, t, c::args, ?loc=r) |> Some
    ...
    | meth, Some c, args ->
        ...
        | "Clear" -> Some "clear"
        | "Remove" -> Some "remove"
        // These are not mutation methods in CSharp
...
let regex com (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "get_Count", Some thisArg ->
        match i.DeclaringEntityFullName with
        | "System.Text.RegularExpressions.GroupCollection" ->
            // In CSharp group count doesn't include group 0 so we need to add 1
