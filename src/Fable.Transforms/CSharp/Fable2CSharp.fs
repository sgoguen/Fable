module rec Fable.Transforms.Fable2CSharp

open System.Collections.Generic
open Fable
open Fable.AST
open Fable.AST.CSharp
...
module Util =
    ...
    let libValue (com: ICSharpCompiler) ctx t moduleName memberName =
        com.GetImportIdent(ctx, memberName, getLibPath com moduleName, t)

    let libTypeRef (com: ICSharpCompiler) ctx moduleName memberName genArgs =
        let ident = libValue com ctx Fable.MetaType moduleName memberName
        Type.reference(ident, genArgs)

    let libCallWithType (com: ICSharpCompiler) ctx t moduleName memberName (args: Expression list) =
    ...
    let libCall (com: ICSharpCompiler) ctx t moduleName memberName (args: Expression list) =
        let t = transformType com ctx t
        libCallWithType com ctx t moduleName memberName args

    let libGenCall (com: ICSharpCompiler) ctx t moduleName memberName (args: Expression list) genArgs =
    ...
    let extLibCall (com: ICSharpCompiler) ctx t modulePath memberName (args: Expression list) =
    ...
    let tryGetEntityIdent (com: ICSharpCompiler) ctx ent =
        CSharp.Replacements.tryEntityIdent com ent
    ...
    let getEntityIdent (com: ICSharpCompiler) ctx (ent: Fable.Entity) =
    ...
    let transformDeclaredTypeIgnoreMeasure ignoreMeasure (com: ICSharpCompiler) ctx (entRef: Fable.EntityRef) genArgs =
        let genArgs = transformGenArgs com ctx genArgs
        ...
        | "Fable.Core.CSharp.CSharpNullable`1", [genArg] -> Nullable genArg |> Some
        ...
        // We use `dynamic` for now because there doesn't seem to be a type that catches all errors in CSharp
    ...
    let transformDeclaredType (com: ICSharpCompiler) ctx (entRef: Fable.EntityRef) genArgs =
    ...
    let hasConstAttribute (atts: Fable.Attribute seq) =
        atts |> Seq.exists (fun att -> att.Entity.FullName = Atts.csharpIsConst)
    ...
    type NamedTailCallOpportunity(_com: ICSharpCompiler, ctx, name, args: Fable.Ident list) =
    ...
    let isInt64OrLess = function
        | Fable.Number(CSharp.Replacements.CSharpInt, _) -> true
        | _ -> false
    ...
    // CSharp linter complaints if we have too many "const"
    ...
    let optimizeTailCall (com: ICSharpCompiler) (ctx: Context) _range (tc: ITailCallOpportunity) args =
    ...
    let transformCallArgs (com: ICSharpCompiler) ctx (info: ArgsInfo) =
    ...
    let getTupleTypeIdent (com: ICSharpCompiler) ctx itemsLength =
        libValue com ctx Fable.MetaType "Types" $"Tuple%i{itemsLength}"

//    let getExceptionTypeIdent (com: ICSharpCompiler) ctx: Ident =
//        transformIdentWith com ctx false Fable.MetaType "Exception"
    ...
    let transformType (com: ICSharpCompiler) (ctx: Context) (t: Fable.Type) =
        match t with
        | Fable.Measure _
        | Fable.Any -> Dynamic // TODO: Object instead? Seems to create issues with CSharp compiler sometimes.
        | Fable.Unit -> Void
    ...
    let transformIdentWith (com: ICSharpCompiler) ctx (isMutable: bool) (typ: Fable.Type) name: Ident =
        let typ = transformType com ctx typ
        makeIdent isMutable typ name

    let transformIdent (com: ICSharpCompiler) ctx (id: Fable.Ident): Ident =
        transformIdentWith com ctx id.IsMutable id.Type id.Name

    let transformIdentAsExpr (com: ICSharpCompiler) ctx (id: Fable.Ident) =
        transformIdent com ctx id |> Expression.identExpression

    let transformGenericParam (com: ICSharpCompiler) ctx (g: Fable.GenericParam): GenericParam option =
    ...
    let transformImport (com: ICSharpCompiler) ctx r t (selector: string) (path: string) =
    ...
    let transformNumberLiteral com r kind (x: obj) =
        match kind, x with
        | CSharp.Replacements.CSharpInt, (:? char as x) -> Expression.integerLiteral(int64 x)
    ...
    let transformTuple (com: ICSharpCompiler) ctx (args: Expression list) =
    ...
    let transformValue (com: ICSharpCompiler) (ctx: Context) (r: SourceLocation option) returnStrategy kind: Statement list * CapturedExpr =
        match kind with
        ...
        // CSharp enums are limited as we cannot set arbitrary values or combine them as flags
        ...
        | Fable.NewOption(expr, genArg, _isStruct) ->
            let transformOption (com: ICSharpCompiler) ctx genArg (arg: Expression) =
        ...
        // We cannot allocate in CSharp without filling the array to a non-null value
    ...
    let transformEmit (com: ICSharpCompiler) ctx t returnStrategy (emitInfo: Fable.EmitInfo) =
    ...
    let typeImplementsOrExtends (com: ICSharpCompiler) (baseEnt: Fable.EntityRef) (t: Fable.Type) =
    ...
    let transformCast (com: ICSharpCompiler) (ctx: Context) targetType returnStrategy (expr: Fable.Expr) =
    ...
    // TODO: Try to identify type testing in the catch clause and use CSharp's `on ...` exception checking
...
//                let test = TypeReference(getExceptionTypeIdent com ctx, [])
    ...
    let transformConditional (com: ICSharpCompiler) ctx returnStrategy guardExpr (thenExpr: Fable.Expr) elseExpr =
    ...
    let transformGet (com: ICSharpCompiler) ctx _range t returnStrategy kind fableExpr =
        match kind with
        | Fable.ExprGet prop ->
    ...
    let transformSet (com: ICSharpCompiler) ctx _range kind toBeSet (value: Fable.Expr) =
    ...
    let transformBinding (com: ICSharpCompiler) ctx (var: Fable.Ident) (value: Fable.Expr) =
    ...
    let transformSwitch (com: ICSharpCompiler) ctx returnStrategy evalExpr cases defaultCase =
        let cases =
        ...
        let cases, defaultCase =
            ...
            | None ->
                // CSharp may complain if we're not covering all cases so turn the last case into default
    ...
    let getDecisionTargetAndBindValues (com: ICSharpCompiler) (ctx: Context) targetIndex boundValues =
    ...
    let transformDecisionTreeSuccess (com: ICSharpCompiler) (ctx: Context) returnStrategy targetIndex boundValues =
        match returnStrategy with
        | Target targetId ->
    ...
    let transformDecisionTreeWithTwoSwitches (com: ICSharpCompiler) ctx returnStrategy
    ...
    let transformDecisionTree (com: ICSharpCompiler) (ctx: Context) returnStrategy
    ...
    let transformTest (com: ICSharpCompiler) ctx _range returnStrategy kind fableExpr =
        transformExprAndResolve com ctx returnStrategy fableExpr (fun expr ->
            match kind with
            | Fable.TypeTest t ->
    ...
    let extractBaseArgs (com: ICSharpCompiler) (ctx: Context) (classDecl: Fable.ClassDecl) =
        match classDecl.BaseCall with
        | Some(Fable.Call(_baseRef, info, _, _) as e) ->
            match transformCallArgs com ctx (CallInfo info) with
            | [], args -> args
            | _, args ->
                $"Rewrite base arguments for {classDecl.Entity.FullName} so they can be compiled as CSharp expressions"
    ...
    let transformAndCaptureExpr (com: ICSharpCompiler) (ctx: Context) (expr: Fable.Expr): Statement list * Expression =
    ...
    let transform (com: ICSharpCompiler) ctx (returnStrategy: ReturnStrategy) (expr: Fable.Expr): Statement list * CapturedExpr =
        match expr with
        ...
        | Fable.Extended(kind, _range) ->
            match kind with
            | Fable.Curry(e, arity) ->
                CSharp.Replacements.curryExprAtRuntime com arity e
        ...
        | Fable.TypeCast(expr, t) ->
            ...
            | Fable.DeclaredType(EntFullName(Types.ienumerableGeneric | Types.ienumerable), [_]) ->
                ...
                | ExprType(Fable.String) ->
                    CSharp.Replacements.stringToCharSeq expr |> transform com ctx returnStrategy

                | ExprType(Fable.DeclaredType(EntFullName "System.Text.RegularExpressions.Match", _)) ->
                    CSharp.Replacements.regexMatchToSeq com t expr |> transform com ctx returnStrategy
    ...
    let getLocalFunctionGenericParams (_com: ICSharpCompiler) (ctx: Context) (_range: SourceLocation option) (argTypes: Fable.Type list): string list =
...
// Sometimes nested generic functions cause issues in CSharp, but I'm not sure of
// the exact conditions so don't display the warning for now
//        match range, genParams with
//        | None, _ | Some _, [] -> ()
//        | Some range, _ -> com.WarnOnlyOnce("Generic nested functions may cause issues with CSharp compiler. Add type annotations or move the function to module scope.", range=range)

        genParams

    let getMemberArgsAndBody (com: ICSharpCompiler) ctx kind (genParams: Fable.GenericParam list) (paramGroups: Fable.Parameter list list) (args: Fable.Ident list) (body: Fable.Expr) =
        let funcName, args, body =
            match kind, args with
            | Attached(isStatic=false), (thisArg::args) ->
                // AFAIK, there cannot be `this` conflicts in CSharp (no class expressions)
    ...
    let getEntityAndMemberArgs (com: ICSharpCompiler) (info: Fable.MemberFunctionOrValue) =
    ...
    let transformModuleFunction (com: ICSharpCompiler) ctx (info: Fable.MemberFunctionOrValue) (memb: Fable.MemberDecl) =
    ...
    let transformAbstractMember (com: ICSharpCompiler) ctx (m: Fable.MemberFunctionOrValue) =
        let kind =
    ...
    let transformInterfaceDeclaration (com: ICSharpCompiler) ctx (decl: Fable.ClassDecl) (ent: Fable.Entity) =
    ...
    // Mirrors CSharp.Replacements.equals
    ...
    // Mirrors CSharp.Replacements.compare
    ...
    let makeStructuralEquals (com: ICSharpCompiler) (ctx: Context) (selfTypeRef: Type) (fields: Ident list): InstanceMethod =
    ...
    let transformFields (com: ICSharpCompiler) ctx (fields: Fable.Field list) =
    ...
    let transformUnionDeclaration (com: ICSharpCompiler) ctx (ent: Fable.Entity) (unionDecl: Fable.ClassDecl) classMethods =
        let genParams = ent.GenericParameters |> List.choose (transformGenericParam com ctx)
    ...
    let transformRecordDeclaration (com: ICSharpCompiler) ctx (ent: Fable.Entity) (decl: Fable.ClassDecl) classMethods =
        let genParams = ent.GenericParameters |> List.choose (transformGenericParam com ctx)
    ...
    let transformAttachedMember (com: ICSharpCompiler) ctx (memb: Fable.MemberDecl) =
    ...
    let transformClass (com: ICSharpCompiler) ctx (classEnt: Fable.Entity) (classDecl: Fable.ClassDecl) classMethods (cons: Fable.MemberDecl option) =
    ...
    let transformDeclaration (com: ICSharpCompiler) ctx decl =
        ...
        | Fable.ActionDeclaration d ->
            "Standalone actions are not supported in CSharp, please use a function"
...
module Compiler =
    ...
    type CSharpCompiler (com: Compiler) =
        let onlyOnceErrors = HashSet<string>()
        let imports = Dictionary<string, Import>()

        interface ICSharpCompiler with
    ...
    let makeCompiler com = CSharpCompiler(com)

    let transformFile (com: Compiler) (file: Fable.File) =
        let com = makeCompiler com :> ICSharpCompiler
        let declScopes =
            let hs = HashSet()
